...
void Canvas::addColorValueToPPMOutput(int &ppmLineLength, float pixelChannelVal, std::string& ppmBodyStr) {
    int channelValue = convertColorFloatToInt(pixelChannelVal);
    channelValue = clampChannelValue(channelValue);
    std::string channelValStr = std::to_string(channelValue); // adding space to the end to separate entries
