...
class Vector3 : public Tuple {
 public:
  ...
  static Vector3 crossProduct(const Vector3& firstV, const Vector3& secondV) {
    return Vector3(
        firstV.y() * secondV.z() - firstV.z() * secondV.y(),
...
class Color : public Tuple {
 public:
  ...
  Color operator+(const Color& rhs) const {
    return Color(
        r() + rhs.r(), g() + rhs.g(), b() + rhs.b()); // , std::min(float(1.0), a() + rhs.a()));
  }

  Color operator-(const Color& rhs) const {
    return Color(
        r() - rhs.r(), g() - rhs.g(), b() - rhs.b()); //, std::max(float(0.0), a() - rhs.a()));
  }
